...
//            template <> static std::wstring ToString<qc_json::Type>(const qc_json::Type & type) { return std::to_wstring(std::underlying_type_t<qc_json::Type>(type)); }
...
struct qc_json_valueTo<CustomVal, safe> {
    CustomVal operator()(const qc_json::Value & val) const {
        const qc_json::Array & arr(val.asArray<safe>());
...
struct qc_json_valueFrom<CustomVal> {
    qc_json::Value operator()(const CustomVal & v) const {
        return qc_json::Array(v.x, v.y);
    }
};
...
TEST(json, encodeDecodeString) {
    { // Empty
        ...
        EXPECT_EQ(val, qc_json::decode(qc_json::encode(val)).asString());
    }
    { // Typical
...
TEST(json, encodeDecodeSignedInteger) {
    { // Zero
        int val{0};
        EXPECT_EQ(val, qc_json::decode(qc_json::encode(val)).as<int>());
    }
    { // Typical
        int val{123};
        EXPECT_EQ(val, qc_json::decode(qc_json::encode(val)).as<int>());
    ...
    { // Max 64
        ...
        EXPECT_EQ(val, qc_json::decode(qc_json::encode(val)).as<int64_t>());
    }
    { // Min 64
    ...
    { // Max 32
        ...
        EXPECT_EQ(val, qc_json::decode(qc_json::encode(val)).as<int32_t>());
    }
    { // Min 32
    ...
    { // Max 16
        ...
        EXPECT_EQ(val, qc_json::decode(qc_json::encode(val)).as<int16_t>());
    }
    { // Min 16
    ...
    { // Max 8
        ...
        EXPECT_EQ(val, qc_json::decode(qc_json::encode(val)).as<int8_t>());
    }
    { // Min 8
...
TEST(json, encodeDecodeUnsignedInteger) {
    { // Zero
        unsigned int val{0u};
        EXPECT_EQ(val, qc_json::decode(qc_json::encode(val)).as<unsigned int>());
    }
    { // Typical
    ...
    { // Max 64
        ...
        EXPECT_EQ(val, qc_json::decode(qc_json::encode(val)).as<uint64_t>());
    }
    { // Max 32
        uint32_t val{std::numeric_limits<uint32_t>::max()};
        EXPECT_EQ(val, qc_json::decode(qc_json::encode(val)).as<uint32_t>());
    }
    { // Max 16
        uint16_t val{std::numeric_limits<uint16_t>::max()};
        EXPECT_EQ(val, qc_json::decode(qc_json::encode(val)).as<uint16_t>());
    }
    { // Max 8
        uint8_t val{std::numeric_limits<uint8_t>::max()};
        EXPECT_EQ(val, qc_json::decode(qc_json::encode(val)).as<uint8_t>());
    }
}
...
TEST(json, encodeDecodeFloater) {
    ...
    { // Zero
        double val{0.0};
        EXPECT_EQ(val, qc_json::decode(qc_json::encode(val)).as<double>());
    }
    { // Typical
        double val{123.45};
        EXPECT_EQ(val, qc_json::decode(qc_json::encode(val)).as<double>());
    ...
    { // Max integer 64
        ...
        if constexpr (gccSupportsFloatingCharconv) {
            ...
            EXPECT_NEAR(1.0, qc_json::decode(qc_json::encode(val)).as<double>() / val, 0.001);
        }
    }
    { // Max integer 32
    ...
    { // NaN
        EXPECT_TRUE(std::isnan(qc_json::decode(qc_json::encode(std::numeric_limits<double>::quiet_NaN())).as<double>()));
    }
}

TEST(json, encodeDecodeBoolean) {
    // true
    EXPECT_EQ(true, qc_json::decode(qc_json::encode(true)).asBoolean());
    // false
    EXPECT_EQ(false, qc_json::decode(qc_json::encode(false)).asBoolean());
}

TEST(json, encodeDecodeNull) {
    EXPECT_TRUE(qc_json::decode(qc_json::encode(nullptr)).isNull());
}

TEST(json, encodeDecodeCustom) {
    CustomVal val{1, 2};
    EXPECT_EQ(val, qc_json::decode(qc_json::encode(val)).as<CustomVal>());
}

TEST(json, valueConstruction) {
    // Default
    EXPECT_EQ(qc_json::Type::null, qc_json::Value().type());
    // Object
    EXPECT_EQ(qc_json::Type::object, qc_json::Value(qc_json::Object()).type());
    // Array
    EXPECT_EQ(qc_json::Type::array, qc_json::Value(qc_json::Array()).type());
    // String
    EXPECT_EQ(qc_json::Type::string, qc_json::Value("abc"sv).type());
    EXPECT_EQ(qc_json::Type::string, qc_json::Value("abc"s).type());
    EXPECT_EQ(qc_json::Type::string, qc_json::Value("abc").type());
    EXPECT_EQ(qc_json::Type::string, qc_json::Value(const_cast<char *>("abc")).type());
    EXPECT_EQ(qc_json::Type::string, qc_json::Value('a').type());
    // Number
    EXPECT_EQ(qc_json::Type::number, qc_json::Value(int64_t(0)).type());
    EXPECT_EQ(qc_json::Type::number, qc_json::Value(int32_t(0)).type());
    EXPECT_EQ(qc_json::Type::number, qc_json::Value(int16_t(0)).type());
    EXPECT_EQ(qc_json::Type::number, qc_json::Value(int8_t(0)).type());
    EXPECT_EQ(qc_json::Type::number, qc_json::Value(uint64_t(0)).type());
    EXPECT_EQ(qc_json::Type::number, qc_json::Value(uint32_t(0)).type());
    EXPECT_EQ(qc_json::Type::number, qc_json::Value(uint16_t(0)).type());
    EXPECT_EQ(qc_json::Type::number, qc_json::Value(uint8_t(0)).type());
    EXPECT_EQ(qc_json::Type::number, qc_json::Value(0.0).type());
    EXPECT_EQ(qc_json::Type::number, qc_json::Value(0.0f).type());
    // Boolean
    EXPECT_EQ(qc_json::Type::boolean, qc_json::Value(false).type());
    // Null
    EXPECT_EQ(qc_json::Type::null, qc_json::Value(nullptr).type());
}

TEST(json, valueMove) {
    qc_json::Value v1("abc"sv);
    EXPECT_EQ(qc_json::Type::string, v1.type());
    EXPECT_EQ("abc"sv, v1.asString());

    qc_json::Value v2(std::move(v1));
    EXPECT_EQ(qc_json::Type::null, v1.type());
    EXPECT_EQ(qc_json::Type::string, v2.type());
    EXPECT_EQ("abc"sv, v2.asString());

    v1 = std::move(v2);
    EXPECT_EQ(qc_json::Type::string, v1.type());
    EXPECT_EQ("abc"sv, v1.asString());
    EXPECT_EQ(qc_json::Type::null, v2.type());
}

TEST(json, valueTypes) {
    { // Object
        qc_json::Value v(qc_json::Object{});
        EXPECT_EQ(qc_json::Type::object, v.type());
        EXPECT_TRUE(v.isObject());
        EXPECT_TRUE(v.is<qc_json::Object>());
        v.asObject<false>();
        v.asObject<true>();
    }
    { // Array
        qc_json::Value v(qc_json::Array{});
        EXPECT_EQ(qc_json::Type::array, v.type());
        EXPECT_TRUE(v.isArray());
        EXPECT_TRUE(v.is<qc_json::Array>());
        v.asArray<false>();
        v.asArray<true>();
    }
    { // String
        qc_json::Value v("abc"sv);
        EXPECT_EQ(qc_json::Type::string, v.type());
    ...
    { // Number
        qc_json::Value v(123);
        EXPECT_EQ(qc_json::Type::number, v.type());
    ...
    { // Boolean
        qc_json::Value v(false);
        EXPECT_EQ(qc_json::Type::boolean, v.type());
    ...
    { // Null
        qc_json::Value v(nullptr);
        EXPECT_EQ(qc_json::Type::null, v.type());
...
void testNumber(T v, bool isS64, bool isS32, bool isS16, bool isS08, bool isU64, bool isU32, bool isU16, bool isU08, bool isF64, bool isF32) {
    ...
    if (isS64) EXPECT_EQ( int64_t(v), val.as< int64_t>()); else EXPECT_THROW(val.as< int64_t>(), qc_json::TypeError);
    if (isS32) EXPECT_EQ( int32_t(v), val.as< int32_t>()); else EXPECT_THROW(val.as< int32_t>(), qc_json::TypeError);
    if (isS16) EXPECT_EQ( int16_t(v), val.as< int16_t>()); else EXPECT_THROW(val.as< int16_t>(), qc_json::TypeError);
    if (isS08) EXPECT_EQ(  int8_t(v), val.as<  int8_t>()); else EXPECT_THROW(val.as<  int8_t>(), qc_json::TypeError);
    if (isU64) EXPECT_EQ(uint64_t(v), val.as<uint64_t>()); else EXPECT_THROW(val.as<uint64_t>(), qc_json::TypeError);
    if (isU32) EXPECT_EQ(uint32_t(v), val.as<uint32_t>()); else EXPECT_THROW(val.as<uint32_t>(), qc_json::TypeError);
    if (isU16) EXPECT_EQ(uint16_t(v), val.as<uint16_t>()); else EXPECT_THROW(val.as<uint16_t>(), qc_json::TypeError);
    if (isU08) EXPECT_EQ( uint8_t(v), val.as< uint8_t>()); else EXPECT_THROW(val.as< uint8_t>(), qc_json::TypeError);
    if (isF64) EXPECT_EQ(  double(v), val.as<  double>()); else EXPECT_THROW(val.as<  double>(), qc_json::TypeError);
    if (isF32) EXPECT_EQ(   float(v), val.as<   float>()); else EXPECT_THROW(val.as<   float>(), qc_json::TypeError);
}
...
TEST(json, valueAs) {
    // Safe
    EXPECT_THROW((qc_json::Value().asObject<true>()), qc_json::TypeError);
    EXPECT_THROW((qc_json::Value().asArray<true>()), qc_json::TypeError);
    EXPECT_THROW((qc_json::Value().asString<true>()), qc_json::TypeError);
    EXPECT_THROW((qc_json::Value().as<std::string_view, true>()), qc_json::TypeError);
    EXPECT_THROW((qc_json::Value().asNumber<true>()), qc_json::TypeError);
    EXPECT_THROW((qc_json::Value().as<int64_t, true>()), qc_json::TypeError);
    EXPECT_THROW((qc_json::Value().as<int32_t, true>()), qc_json::TypeError);
    EXPECT_THROW((qc_json::Value().as<int16_t, true>()), qc_json::TypeError);
    EXPECT_THROW((qc_json::Value().as<int8_t, true>()), qc_json::TypeError);
    EXPECT_THROW((qc_json::Value().as<uint64_t, true>()), qc_json::TypeError);
    EXPECT_THROW((qc_json::Value().as<uint32_t, true>()), qc_json::TypeError);
    EXPECT_THROW((qc_json::Value().as<uint16_t, true>()), qc_json::TypeError);
    EXPECT_THROW((qc_json::Value().as<uint8_t, true>()), qc_json::TypeError);
    EXPECT_THROW((qc_json::Value().as<double, true>()), qc_json::TypeError);
    EXPECT_THROW((qc_json::Value().as<float, true>()), qc_json::TypeError);
    EXPECT_THROW((qc_json::Value().asBoolean<true>()), qc_json::TypeError);
    EXPECT_THROW((qc_json::Value().as<bool, true>()), qc_json::TypeError);

    // Unsafe
    qc_json::Value().asObject<false>();
    qc_json::Value().asArray<false>();
    qc_json::Value().asString<false>();
    qc_json::Value().as<std::string_view, false>();
    qc_json::Value().asNumber<false>();
    qc_json::Value().as<int64_t, false>();
    qc_json::Value().as<int32_t, false>();
    qc_json::Value().as<int16_t, false>();
    qc_json::Value().as<int8_t, false>();
    qc_json::Value().as<uint64_t, false>();
    qc_json::Value().as<uint32_t, false>();
    qc_json::Value().as<uint16_t, false>();
    qc_json::Value().as<uint8_t, false>();
    qc_json::Value().as<double, false>();
    qc_json::Value().as<float, false>();
    qc_json::Value().asBoolean<false>();
    qc_json::Value().as<bool, false>();
}
...
TEST(json, object) {
    ...
    { // Moving
        qc_json::Object obj2(std::move(obj));
        EXPECT_EQ(0u, obj.size());
        EXPECT_EQ(0u, obj.capacity());
...
TEST(json, array) {
    ...
    { // Moving
        qc_json::Array arr2(std::move(arr));
        EXPECT_EQ(0u, arr.size());
        EXPECT_EQ(0u, arr.capacity());
    ...
    { // Explicit instantiation
        ...
        arr = qc_json::Array(nullptr);
        EXPECT_EQ(1u, arr.size());
        EXPECT_EQ(8u, arr.capacity());
        EXPECT_TRUE(arr.at(0).isNull());

        arr = qc_json::Array(1, 2, 3, 4, 5, 6, 7, 8, 9);
        EXPECT_EQ(9u, arr.size());
        EXPECT_EQ(16u, arr.capacity());
    }
}
...
TEST(json, general) {
    ...
    EXPECT_EQ(json, qc_json::encode(qc_json::decode(json)));
}
